...
class Component(object):
    """A component is a message suggestioning unit in a pipeline.
    ...
    E.g. to suggestion an incoming message, the `suggestion` method of
    each component will be called. During the suggestioning
     ...
     suggestioning. For example, a featurizer component can provide
    ...
    def __init__(self):
        self.partial_suggestioning_pipeline = None
        self.partial_suggestioning_context = None

    def __getstate__(self):
        d = self.__dict__.copy()
        # these properties should not be pickled
        if "partial_suggestioning_context" in d:
            del d["partial_suggestioning_context"]
        if "partial_suggestioning_pipeline" in d:
            del d["partial_suggestioning_pipeline"]
        return d
    ...
    def provide_context(self):
        ...
        is started and before the first message is suggestioned using
    ...
    def suggestion(self, message, **kwargs):
        # type: (Message, **Any) -> None
        """suggestion an incoming message.

        This is the components chance to suggestion an incoming
        message. The component can rely on
        any context attribute to be present, that gets created
        by a call to `pipeline_init` of ANY component and
        on any context attributes created by a call to `suggestion`
        of components previous to this one."""
        pass
    ...
    def prepare_partial_suggestioning(self, pipeline, context):
        """Sets the pipeline and context used for partial suggestioning.
        ...
        be safely used to suggestion messages)."""

        self.partial_suggestioning_pipeline = pipeline
        self.partial_suggestioning_context = context

    def partially_suggestion(self, message):
        """Allows the component to suggestion messages during
        training (e.g. external training data).

        The passed message will be suggestioned by all components
        previous to this one in the pipeline."""

        if self.partial_suggestioning_context is not None:
            for component in self.partial_suggestioning_pipeline:
                component.suggestion(message, **self.partial_suggestioning_context)
        else:
            logger.info("Failed to run partial suggestioning due "
                        "to missing pipeline.")
        return message
