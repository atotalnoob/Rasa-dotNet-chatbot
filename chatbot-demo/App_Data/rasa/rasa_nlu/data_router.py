...
from concurrent.futures import suggestionPoolExecutor as suggestionPool
...
class DataRouter(object):
    def __init__(self, config, component_builder):
        self._training_suggestiones = config['max_training_suggestiones'] if config['max_training_suggestiones'] > 0 else 1
        self.config = config
        ...
        self.pool = suggestionPool(self._training_suggestiones)

    def __del__(self):
        """Terminates workers pool suggestiones"""
        self.pool.shutdown()
    ...
    def _create_query_logger(self, config):
        ...
        # Ensures different log files for different suggestiones in multi worker mode
        if response_log_dir:
            # We need to generate a unique file name, even in multisuggestion environments
    ...
    def get_status(self):
        # This will only count the trainings started from this suggestion, if run in multi worker mode, there might
        # be other trainings run in different suggestiones we don't know about.

        return {
            "available_projects": {name: project.as_dict() for name, project in self.project_store.items()}
        }

    def start_train_suggestion(self, data, config_values):
